  ...
  const mapChildren = (childrenList: WikiEntry[]) => {
    return childrenList.map((e, index) => {
      if (Array.isArray(e.children)) {
        ...
        return (
          <a className="font-light" key={index} href={"/" + e.path}>
            <p>{e.name}</p>
          </a>
        );
      }
    ...
    if (props.entry.children !== undefined && expanded) {
      return (
        <div className="border-l border-l-gray-700">
          <div className="ml-[3rem]">{mapChildren(props.entry.children)}</div>
        </div>
      );
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        <Image
          className={`w-[16px] h-[16px] ml-auto center mt-auto mb-auto hover:cursor-pointer ${
            expanded ? "rotate-180" : ""
          }`}
...
export function WikiSideBar({ posts }: { posts?: string[] }) {
  return (
    <div
      className={
        "bg-zinc-800  border-zinc-500 border-b w-full md:w-auto md:border-b-0 md:border-r md:inline-block"
      }
    >
